...
def msa_multi_root(scores):
    ...
    if not cycles:
        ...
        contracted_tree = msa_multi_root(subscores)
...
#===============================================================
def msa_one_root(scores):
    """"""

    scores = scores.astype(np.float64)
    tree = msa_multi_root(scores)
...
def parse(weights, one_root=True, adjacency_matrix=True):
    if len(weights.shape) == 3:
        weights, label_argmax = torch.max(weights, dim=2)
        weights = weights.cpu()
        label_argmax = label_argmax.cpu()
    else:
        weights = weights.cpu()
        label_argmax = None
    ...
    heads = msa_one_root(weights) if one_root else msa_multi_root(weights)

    ret = list()
    for mod in range(1, len(heads)):
        head = heads[mod]
        if label_argmax is None:
            ret.append((head, mod, -1))
        else:
            ret.append((head, mod, label_argmax[head, mod].item()))
    return ret